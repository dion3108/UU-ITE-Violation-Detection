...
def get_tiktok_cookies():
    ...
    for browser in browsers:
        try:
            cookies = browser(domain_name='.tiktok.com')
            if cookies:
                print(f"✅ Successfully loaded cookies from {browser.__name__}")
                return cookies
        except Exception as e:
            continue
    print("⚠️ Warning: Could not get TikTok cookies from browsers. Please ensure you're logged into TikTok.")
    return None

def create_session():
    """Create and configure a requests session with enhanced headers."""
...
def req(post_id, curs, session, retry_count=0):
    """API request with improved retries and exponential backoff."""
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    print(f"⚠️ Invalid JSON response on attempt {attempt + 1}")
                    continue
                ...
                print(f"\n⚠️ Access forbidden (403). Cookies may be invalid or expired.")
                return {"comments": [], "has_more": 0}
                
            else:
                print(f"\n⚠️ HTTP {response.status_code} on attempt {attempt + 1}")
                
        except requests.exceptions.Timeout:
            print(f"\n⚠️ Request timeout on attempt {attempt + 1}")
        except requests.exceptions.ConnectionError as e:
            print(f"\n⚠️ Connection error on attempt {attempt + 1}: {str(e)[:50]}")
        except Exception as e:
            print(f"\n⚠️ Unexpected error on attempt {attempt + 1}: {str(e)[:100]}")
            
        if attempt == 2:
            print(f"❌ Final request failure after 3 attempts")
...
def get_all_replies(comment_id, post_id, session):
    ...
    for _ in range(5):  # Max 5 pages
        ...
        if not data.get('has_more', 0):
            break
        reply_cursor += 20
        time.sleep(0.5 + random.uniform(0.1, 0.3))  # Increased delay
...
def scrape_single_tiktok(url, session):
    ...
    try:
        ...
        for page in range(200):  # Max pages
            if exit_event.is_set():
                return []

            data = req(post_id, curs, session, retry_count=consecutive_failures)
            ...
            if not batch_comments:
                ...
                if consecutive_empty >= 3:
                    print(f"\n⚠️ No more comments found after {page} pages")
                    break
                    
                # Longer delay when getting empty responses
            ...
            # Process comments with reduced parallelism to avoid rate limits
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        print(f"\n⚠️ Error processing comment: {str(e)[:100]}")
            ...
            if data.get('has_more', 0):
                ...
                if new_cursor and new_cursor != curs:
                    curs = new_cursor
                else:
                    # Fallback to increment if no valid cursor
                    curs += 20
            else:
                print(f"\n✅ Reached end of comments (has_more=0)")
                break
        ...
        print(f"\n✅ Done scraping {url}. Total collected: {len(comments)}")
        return comments
        ...
        with save_lock:
            ...
            update_progress_display()
        print(f"\n❌ Error processing {url}: {str(e)[:200]}")
        return []
...
def signal_handler(sig, frame):
    ...
    if all_comments:
        ...
        print(f"💾 Saved {len(all_comments)} comments to final_save.csv")
    sys.exit(0)

def scrape_multiple_tiktoks(urls, max_workers=3):
    ...
    print(f"⚠️  Make sure you're logged into TikTok in your browser!\n")
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                url = futures[future]
                try:
                    future.result(timeout=900)  # Increased timeout
                except concurrent.futures.TimeoutError:
                    print(f"\n⚠️ Timeout processing {url}")
                except Exception as e:
                    print(f"\n❌ Task failed for {url}: {str(e)[:200]}")
    finally:
        session.close()
    ...
    final_filename = 'tes(7).csv'
    save_to_csv(all_comments, final_filename)
...
if __name__ == "__main__":
    tiktok_urls = [
        ...
        # 'https://www.tiktok.com/@udzrieaulia/video/7467920197777706246',
        'https://www.tiktok.com/@kyshaaa02_/video/7549780272535981320'
    ]
    
    try:
        # Reduced from 7 to 3 workers to avoid aggressive rate limiting
        scrape_multiple_tiktoks(tiktok_urls, max_workers=3)
    except KeyboardInterrupt:
        print("\n\nUser interrupted. Saving data and exiting...")
        if all_comments:
            save_to_csv(all_comments, 'interrupted_save.csv')
            print(f"💾 Saved {len(all_comments)} comments to interrupted_save.csv")
        os._exit(0)